...
**Version pin (combined skill hash):** `704577cca97e8dc8`
**Status:** active
...
These evals operationalize PE-1 through PE-21 from `docs/implementation-authoring/delivery-pipeline/20-plan-epic.md`. A passing result needs concrete artifact evidence, not a bare PASS claim.
...
| PE-12 | DAG frozen only after whole-graph event/record producer reconciliation and phantom-consumer rejection: every event/record consumed by any story maps to one declared producer node (this epic or a prior frozen one), every listed consumer of a current-DAG producer has a labelled dependency edge plus a DAG-declared consumed shape/predicate, and consumers of prior frozen producers are covered by reconciliation rows naming the frozen source. An orphaned consumed event or phantom consumer is a Gate 3 failure, not deferred to per-story checks. | `pe-dag-level-closure` presents a plan where one story consumes `ApprovalDecisionRecorded` but no story declares it as a produced output; the output must flag it as a DAG-level closure defect, refuse to freeze the DAG, and refuse `story: ready` for the consumer until a producer is named or the gap is escalated. `pe-phantom-consumer-edge` rejects prose-only consumers before contracts exist. |
...
| PE-19 | Refuses `story: ready` when any manifest item lacks `manifest item -> AC -> standing gate lane`, or when non-command evidence lacks a concrete range, fixture id, or generated artifact id. | `pe-manifest-gate-lane-coverage` presents a manifest item with only prose/manual proof and an evidence pack entry missing concrete reconstruction data; the output must flag Gate 4/PD-12 for the orphaned manifest obligation and Gate 5/LSN-15 for incomplete evidence. |
| PE-20 | Refuses `story: ready` when a pure/value/classifier/projection story owns writer, append, persistence, or event-log obligations without explicitly owning the writer seam. | `pe-pure-classifier-writer-boundary` presents a classifier-only story that must append an event-log record without naming a writer seam; the output must classify this as a Gate 4 boundary contradiction and refuse readiness. |
| PE-21 | Refuses `story: ready` when an unattended recovery, clear, apply, auto-retry, or similar safety action does not name the classification producer and required committed gate record. | `pe-safety-action-provenance` presents an unattended stale-clear action backed only by a prose safety label; the output must require the classification producer plus committed `auto-recover` or relevant gate record, or make the action manual-only. |
