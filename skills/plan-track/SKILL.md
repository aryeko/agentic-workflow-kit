---
name: plan-track
description: Use after a PRD exists to decompose it into a delivery tracker — when the user asks to "plan the track", "decompose this PRD", "spec the rollout/migration/refactor", "break this into stories", "create a tracker", or whenever agreed product work is too large for one session and splits into 5-25 bounded stories that may be picked up across sessions or in parallel. Consumes a PRD (docs/prds/<slug>/, per references/prd-contract.md) and produces a tracker conforming to references/tracker-contract.md (Mermaid dependency graph, waves, parallelism rules, ID-prefix registry) plus per-story standalone (Pattern A) and delta (Pattern B) specs, mapping stories back to the PRD's PREFIX-n acceptance criteria. If no PRD exists, it stops and points at /plan-product. Do NOT use for one-shot single-PR work (write a plan instead), for authoring the product definition itself (use /plan-product), or for executing a story (use /implement-next).
...
`plan-product` (what/why → a PRD) → **`plan-track` (how → tracker + specs)** → `implement-next`
(per-story execution) → orchestrator (autonomous multi-session). `plan-track` reads the PRD via
`references/prd-contract.md` and emits the tracker via `references/tracker-contract.md`. The PRD
stays the authoritative source of done-ness (stories cite its `PREFIX-n` acceptance criteria);
the tracker is the single source of truth for story *state*.
...
Ten steps. The PRD gate (Step 1) and the reality audit (Step 2) are the most-skipped and most
load-bearing — every later step compounds an error there.
...
### Step 2 — Audit reality (best-effort)
...
too big). Map each story to one or more PRD `PREFIX-n` criteria.

### Step 5 — Dependency graph + waves + write the tracker README
...
### Step 8 — Define the validation gate
...
### Step 9 — Living playbook & cross-track coordination
...
to the tracker and cites PRD IDs. Point the user at `/implement-next` as the next step. Do **not**
auto-commit, and do **not** invoke `writing-plans` or any execution skill — that crosses into
the implementer's altitude.
