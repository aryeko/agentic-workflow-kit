# Implementer Prompt: core-02-s2-gate-evaluator
...
Implement source story `core-02-s2-gate-evaluator` for epic `epic-3-core-runtime-spine`. Deliver exactly the outcome in the ready source contract and nothing outside it:
...
`CapabilityGatePolicyDecision`, `GuaranteeEvaluation`, `AttestationRef`, `CapabilityGateScope`,
`GateDecision`, `ProviderDomain`), and the full `CapabilityGateFailureReason` union — implemented as a pure fail-closed decision over the
...
Do not stop on the old policy-shape or approved-parent-scope gap; it is resolved by the repaired source
story and the matching design docs.
...
The DAG dependents for this story are: `core-02-s3-gate-record-durability`. Preserve the producer/consumer shape boundaries named above so later stories can consume committed dependency inputs without re-declaring or widening this story's scope.

## Required Reading

- `docs/implementation/epics/epic-3-core-runtime-spine/stories/core-02-s2-gate-evaluator.md` — source story contract for `core-02-s2-gate-evaluator`.
- `docs/implementation/epics/epic-3-core-runtime-spine/story-dag.md` — frozen DAG row, dependencies, owned pathset, wave, and suggested-tier floor for `core-02-s2-gate-evaluator`.
...
Source story: `docs/implementation/epics/epic-3-core-runtime-spine/stories/core-02-s2-gate-evaluator.md`. Source AC ids: `AC-1`, `AC-2`, `AC-3`, `AC-4`, `AC-5`, `AC-6`, `AC-7`, `AC-8`, `AC-9`, `AC-10`, `AC-11`, `AC-12`, `AC-13`, `AC-14`, `AC-15`, `AC-16`, `AC-17`, `AC-18`.
...
Source story: `docs/implementation/epics/epic-3-core-runtime-spine/stories/core-02-s2-gate-evaluator.md`. Owned pathset from the frozen DAG and source contract:

- `packages/sdk/src/core/capability/evaluator/**`
- `packages/sdk/tests/core/capability/evaluator/**`
...
Direct dependency story ids: `core-02-s1-capability-registry`, `core-01-s1-event-contracts`.
...
- Apply the attestation-consumption checks: valid envelope fields; freshness `at <= evaluatedAt <
  ...
  parentage, kill, egress confinement, or write-side Forge behavior. Exact-or-approved-parent is defined
  by the source story's `approvedParentScopes` rule.
...
Return a report with changed files, AC coverage by `AC-1`, `AC-2`, `AC-3`, `AC-4`, `AC-5`, `AC-6`, `AC-7`, `AC-8`, `AC-9`, `AC-10`, `AC-11`, `AC-12`, `AC-13`, `AC-14`, `AC-15`, `AC-16`, `AC-17`, `AC-18`, tests and checks run, evidence pack, open questions, and blockers. The report is review evidence only; it is not permission to update tracker state or perform delivery actions.
...
**↑ Up:** [Epic 3 - Core runtime spine](../../../README.md) · **← Prev:** [Reviewer Prompt: core-02-s1-capability-registry](../core-02-s1-capability-registry/reviewer.md) · **Next →:** [Reviewer Prompt: core-02-s2-gate-evaluator](./reviewer.md)
