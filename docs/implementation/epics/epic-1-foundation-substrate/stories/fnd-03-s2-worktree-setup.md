...
- Coverage command and instrumented lane(s): integration helper scope must be measured with `pnpm exec vitest run --project integration --coverage --passWithNoTests -- packages/sdk/tests/foundation/workspace-repository/worktree/*.int.test.ts`; unit-only `pnpm coverage:baseline` is not sufficient for this story.
...
- Exact commands: `pnpm test:int -- packages/sdk/tests/foundation/workspace-repository/worktree/*.int.test.ts`;
  `pnpm check`; coverage with `pnpm exec vitest run --project integration --coverage --passWithNoTests -- packages/sdk/tests/foundation/workspace-repository/worktree/*.int.test.ts` for integration helper scope.
