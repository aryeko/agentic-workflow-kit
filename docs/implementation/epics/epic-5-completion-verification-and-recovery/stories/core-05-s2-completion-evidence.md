---
title: "core-05-s2-completion-evidence implementation story"
id: "core-05-s2-completion-evidence"
epic: 5
status: "story: ready"
design:
...
# core-05-s2-completion-evidence
...
|---|---|---|---|---|
| AC-1 | latest usable clean exact head | `LocalGitEvidence.headSha`, `clean`, active lease ref, `evaluatedThrough.afterSequence` | fnd-03 + core-01 cursor | decidable |
| AC-3 | path inside allowlist/protected/evidence sets | `LocalGitEvidence.changedPaths`, fnd-01 allowlist, protected snapshot path-set digests | fnd-01, fnd-03, owned snapshot | decidable |
| AC-3 | protected policy approved | `ApprovalDecisionRecorded.subject === "protected-policy-change"` and approved decision | Epic 4 core-03 | decidable |
| AC-4 | verify command freshness | `RunnerCommandCaptured` fields plus pre/post local git evidence | Execution Host and fnd-03 evidence | decidable |
| AC-5 | worker claim mismatch | worker claim event text/class plus independent local/verification evidence state | replay event refs | decidable |
| AC-6 | append success/failure | `RunWriter.appendBarrier` result | core-01 writer | decidable |
| AC-8 | independent completion evidence absent | selected clean head plus absence of usable verification/local evidence refs | replay event refs and `core-05-s1` catalog | decidable |
| AC-9 | merge-readiness claim lacks Forge evidence | worker claim kind plus Forge evidence availability for exact head | replay event refs and Forge evidence refs | decidable |

### Produced Obligations

| Produced record/event/symbol | Required field or symbol | Declared source | Verdict |
|---|---|---|---|
| `ProtectedPolicySnapshotRecorded` | `schema` | constant schema string | closed |
| `ProtectedPolicySnapshotRecorded` | `policyRef`, `baseSha`, `verifierCommandDigest`, path-set digests | resolved policy input, fnd-03 base evidence, verify command configuration | closed |
| `CompletionDecisionRecorded` | `runId`, `state`, `headSha`, `cursor`, `evidenceRefs`, `evaluatedAt` | request input, owned evaluator result, selected local git evidence, cursor input, evidence refs, injected clock | closed |
...
| `verification-failed` | complete verify capture exits nonzero | completion not verified | AC-4 |
| `verification-uncertain` | incomplete capture, host failure, digest mismatch, or head mismatch | completion not verified | AC-4 |
| `claim-evidence-mismatch` | worker claim lacks matching independent evidence | record mismatch state | AC-5 |
| `completion-pending-evidence` | clean candidate head exists but required independent completion evidence is absent without a contradictory claim | record pending state | AC-8 |
| `forge-evidence-unavailable` | worker claim explicitly asserts merge readiness but required Forge-side evidence is missing or unavailable | record Forge evidence unavailable for that claim | AC-9 |
| `event-log-unwritable` | completion decision append fails | return unwritable failure, no success fact | AC-6 |
...
The `packages/sdk/src/core/completion/evidence/**` module, tests, fixtures, and SDK export lines.
...
- Design -> AC completeness: candidate head, protected snapshot, changed-file gate, verification
  freshness, claim mismatch, pending evidence, Forge-evidence unavailable, and event-log unwritable
  invariants map to AC-1..AC-9.
- Producer closure: every produced event field has a source row.
- Sweep vocabulary: forbidden tokens do not ban normative state literals.
- Failure-token/catalog closure: all consumed tokens are from `core-05-s1`.
- Verdict: ready.
...
**↑ Up:** [Epic 5 - stories](./README.md) · **← Prev:** [core-05-s1-completion-contracts implementation story](./core-05-s1-completion-contracts.md) · **Next →:** [core-05-s3-merge-readiness implementation story](./core-05-s3-merge-readiness.md)
