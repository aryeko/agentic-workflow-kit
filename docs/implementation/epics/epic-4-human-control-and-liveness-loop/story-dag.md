...
- Edge = an intra-epic dependency because a consumer story uses a shared shape or behavior produced by
  another story. Cross-epic frozen inputs are named in story contracts, not shown as intra-epic edges.
...
| `core-03-s4-grant-mapping-and-outcome` | Map policy grants to Agent `ScopedGrant`, answer or deny through Agent approval relay, and record outcome audit facts. | `core-03` | Mapping from policy-level grants to Agent-provider scoped grants; `split`: relay/channel/mapping/outcome fail-closed behavior; neutral records behavior part. | `packages/sdk/src/core/approval/grants/**`, `packages/sdk/src/core/approval/outcomes/**`, `packages/sdk/tests/core/approval/grants/**`, `packages/sdk/tests/core/approval/outcomes/**` | elevated |
...
| `core-04-s2-liveness-fold` | Implement the pure liveness fold over committed event values plus sampled clock, including advancing and never-refresh event classes. | `core-04` | Liveness state fold over committed events and explicit clock input; Current-session event classes that advance liveness; Event classes that explicitly never refresh liveness. | `packages/sdk/src/core/supervision/liveness/**`, `packages/sdk/tests/core/supervision/liveness/**` | elevated |
| `core-04-s3-timers-and-wait` | Evaluate startup/idle/no-progress/per-tool/approval-SLA/max-runtime timers and wrap the Epic 3 cursor wait primitive without liveness side effects. | `core-04` | Startup, idle, no-progress, per-tool, approval-SLA, and max-runtime timer signals; `waitRunEvents` wrapper behavior and cursor validation. | `packages/sdk/src/core/supervision/timers/**`, `packages/sdk/src/core/supervision/wait/**`, `packages/sdk/tests/core/supervision/timers/**`, `packages/sdk/tests/core/supervision/wait/**` | elevated |
| `core-04-s4-termination-handoff` | Record supervisor lifecycle, supervision-lost, termination-requested, worker-terminated, and supervisor-stopped facts through `RunWriter` and Execution Host handoff. | `core-04` | Supervisor start, liveness advanced, timer expired, supervision lost, termination requested, worker terminated, and supervisor stopped facts (behavior part); `split`: cursor/linkage/progress/stale/termination fail-closed behavior. | `packages/sdk/src/core/supervision/termination/**`, `packages/sdk/tests/core/supervision/termination/**` | elevated |

## Dependency table

| story | depends on | shared contract creating the edge |
|---|---|---|
| `core-03-s1-approval-contracts` | none | Producer of `ApprovalRequest`, `ApprovalMode`, `ApprovalRisk`, `ApprovalState`, `ApprovalSubject`, `PolicyGrantScope`, `PolicyGrantPlan`, `Decision`, `Outcome`, `ResumeDecision`, `ApprovalFailureState`, approval event payloads, `ApprovalProjection`, and approval interfaces. |
| `core-03-s2-risk-and-decision` | `core-03-s1-approval-contracts` | Consumes approval value types, `ApprovalDecisionInput`, `Decision`, `ApprovalFailureState`, and policy grant taxonomy; produces normalization, risk classifier, and decision behavior. |
| `core-03-s3-pending-park-resume` | `core-03-s1-approval-contracts`, `core-03-s2-risk-and-decision` | Consumes approval event/projection shapes, `Decision`, `ResumeDecision`, and deadlines; produces pending persistence, park/resume/expiry behavior, and projection folds. |
| `core-03-s4-grant-mapping-and-outcome` | `core-03-s1-approval-contracts`, `core-03-s2-risk-and-decision`, `core-03-s3-pending-park-resume` | Consumes `PolicyGrantPlan`, `Decision`, pending/resume facts, `Outcome`, and Agent `ScopedGrant`; produces grant mapping and outcome recording behavior. |
| `core-04-s1-supervision-contracts` | none | Producer of `SupervisionInputs`, `Clock`, `SupervisionTimerPolicy`, `SupervisionWaitRequest`, `SupervisionTimerName`, `LivenessAdvanceClass`, `LivenessState`, `LivenessReason`, `LivenessProjection`, supervision event payloads, and termination fact shapes. |
| `core-04-s2-liveness-fold` | `core-04-s1-supervision-contracts` | Consumes liveness value types, timer names, advance classes, and event payload shapes; produces the pure liveness reducer and non-refresh catalog. |
| `core-04-s3-timers-and-wait` | `core-04-s1-supervision-contracts`, `core-04-s2-liveness-fold` | Consumes timer policy, `LivenessProjection`, `SupervisionWaitRequest`, and Epic 3 `RunEventCursor`; produces timer evaluation and `waitRunEvents` wrapper behavior. |
| `core-04-s4-termination-handoff` | `core-04-s1-supervision-contracts`, `core-04-s2-liveness-fold`, `core-04-s3-timers-and-wait` | Consumes liveness/timer facts, supervisor event payloads, and Execution Host termination DTOs; produces supervision-lost and termination handoff behavior. |
...
|---|---|---|
| 1 | `core-03-s1-approval-contracts`, `core-04-s1-supervision-contracts` | Root value-type, event, projection, and failure catalogs. Independent within the band; both expose public SDK shapes. |
| 2 | `core-03-s2-risk-and-decision`, `core-04-s2-liveness-fold` | First behavior consumers of the contract stories. Both are pure reducers/deciders over declared inputs and fixtures. |
| 3 | `core-03-s3-pending-park-resume`, `core-04-s3-timers-and-wait` | Durable approval pending/resume behavior and supervision timer/wait behavior. Independent within the band. |
| 4 | `core-03-s4-grant-mapping-and-outcome`, `core-04-s4-termination-handoff` | Provider-port handoff behaviors over Agent and Execution Host ports. Independent within the band. |
...
- **Single producer per shared shape.** Approval value types are produced by `core-03-s1`; supervision
  ...
  them.
- **Acyclic, labelled edges.** The graph has four topological bands and every edge names the shape or
