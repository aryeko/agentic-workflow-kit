...
- Dependencies: `core-03-s1-approval-contracts` (intra-epic); cross-epic on `core-01-r2-run-launch-worktree-path` (in `epic-r2-run-launch-workspace-fact`, frozen earlier) for the trusted `RunLaunchProjection.worktreePath` workspace boundary.
- Dependency inputs: `{{DEPENDENCY_COMMITS}}` and committed producer shapes/events named by the source DAG and story contract, including `RunLaunchProjection.worktreePath` produced by `core-01-r2-run-launch-worktree-path` (consumed as `ApprovalContext.worktreePath`, the dependency-commit input for the workspace boundary).
...
- **Workspace-containment boundary (blocking — fixes the P1 fail-open):** both operands of the AC-3 (`ApprovalRequest.filePaths` vs workspace root) and AC-4 (`ApprovalRequest.cwd` vs workspace root) containment predicates must be sourced from declared fields — the tested point from `ApprovalRequest.filePaths` / `ApprovalRequest.cwd` and the boundary from the trusted `ApprovalRequest.worktreePath` (copied by `normalize` from `ApprovalContext.worktreePath`, whose value is the recorded `RunLaunchProjection.worktreePath` produced by `core-01-r2-run-launch-worktree-path`). Block any implementation that approximates, derives, or substitutes the workspace boundary from the agent-supplied `cwd` or any agent request path (the fail-open), or that leaves either operand of a containment predicate unsourced. Require fail-closed behavior when `worktreePath` is absent: AC-3 "outside the workspace" triggers for requests carrying `filePaths`, and AC-4's low-risk "cwd inside the workspace" condition is unmet (no low-risk auto-grant). Confirm AC-1 copies `worktreePath` into the normalized `ApprovalRequest` from `ApprovalContext` (trusted), never from the agent request.
