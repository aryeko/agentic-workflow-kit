...
- Dependencies: `core-03-s1-approval-contracts` (intra-epic) for approval fields; the approval composition assembler supplies `ApprovalContext.worktreePath` from the run's `WorktreeLease.worktreePath`.
- Dependency inputs: `{{DEPENDENCY_COMMITS}}` and committed producer shapes/events named by the source DAG and story contract, including `ApprovalContext.worktreePath` copied into `ApprovalRequest.worktreePath`.
