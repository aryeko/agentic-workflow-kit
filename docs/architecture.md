...
Tool-specific descriptions stay with the individual MCP tools. The Codex plugin `mcpServers`
...
.mcp.json                   plugin MCP wiring using Codex's `mcpServers` shape; Codex entries omit `cwd`
...
plugins/agentic-workflow-kit/       materialized copy for the local Codex marketplace fixture, including
                                    Codex-specific .mcp.json (`mcpServers`, no `cwd`) and mcp/server.mjs
