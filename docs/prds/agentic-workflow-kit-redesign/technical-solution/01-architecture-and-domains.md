...
| Agent execution | Agent profiles, prompt rendering, model/reasoning selection, driver capability negotiation, child session lifecycle. | Tracker parsing or report generation. | `ResolvedAgentProfile`, prompt payloads, `StoryRunner`, Codex driver events |
...
| `ConfigSchema` extensions | Add agent profiles, task bindings, prompt defaults, structured-output contracts, budgets, notification/watch policy, artifact/report settings, and migration settings. | `.workflow/config.yaml` | Resolved config plus JSON schema/docs | Zod schema, config docs, presets |
| Agent profile resolver | Resolve logical task type to a named profile, then resolve prompt/template, model, reasoning effort, structured output contract, approval policy, sandbox, host config, and budget defaults. | Config, CLI/MCP overrides, task type, optional profile override | Effective child/session launch policy | Config loader, CLI/MCP schemas |
