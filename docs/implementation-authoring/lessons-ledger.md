...
| ID | Source | Lesson | Covered by | Status |
|----|--------|--------|-----------|--------|
| LSN-08 | #127 | Root-barrel "ownership void": public shapes never exposed on the SDK surface. Reconciled barrel model: behavior stories declare public-exposure ACs (export + import path + import test) but do NOT include `packages/sdk/src/index.ts` in their owned pathsets; the barrel is updated by the implementer per those ACs. A story that claims barrel ownership in its pathset is wrong. | Public-exposure dimension (R3, Quality bar, Gate 4) — [authoring-standard/50-story-contract.md](authoring-standard/50-story-contract.md); barrel-ownership rule — [authoring-standard/40-story-dag.md](authoring-standard/40-story-dag.md) | covered |
...
| LSN-25 | Epic 4 re-plan / barrel pathsets | Multiple stories included `packages/sdk/src/index.ts` in their owned pathsets, reproducing the barrel-ownership defect that Phase A addressed only in design prose. The authoring standard contradicted the design by saying "pathsets may overlap on a barrel… not a shared-file owner" and not naming the barrel as unowned. | Barrel-ownership rule in [authoring-standard/40-story-dag.md](authoring-standard/40-story-dag.md); PE-17 eval; updated LSN-08. | covered |
