...
| ID | Source | Lesson | Covered by | Status |
|----|--------|--------|-----------|--------|
| LSN-24 | Epic 4 re-plan / `ApprovalDecisionRecorded` | An event consumed by multiple stories was produced by none. Per-story closure cannot catch this: if no story claims production, each story's own check passes. The same graph-level gap appears when a DAG claims a consumer that has no dependency edge and no DAG-declared consumed-shape/predicate use: a phantom consumer edge makes ordering look real while no DAG artifact consumes the producer. Requires whole-graph producer/consumer reconciliation at the DAG level before freeze. | Gate 3 whole-graph event/record producer reconciliation + no-phantom-consumers check — [authoring-standard/40-story-dag.md](authoring-standard/40-story-dag.md); PE-12 eval — [delivery-pipeline/20-plan-epic.md](delivery-pipeline/20-plan-epic.md) | covered |
...
| LSN-30 | Epic 4 / `core-03-s2` | A relational safety predicate ("cwd **inside the workspace**") declared only one of two operands as a frozen input — the workspace-root operand was unsourced, so the predicate is undecidable; a tests-passing approximation (`cwd` as its own root) silently **failed open** (spoofable bypass of the sibling AC-3 high-risk rule). Stale unattended clear/recovery actions are this same predicate-input closure problem with a safety-action specialization: the action must name the classification producer and the committed gate record it consumes, not a stale or prose classification. Explicit **granularity-strengthening of LSN-21** from per-AC to **per-sub-predicate**, with relational **two-operand** sourcing and **field-not-category** closure rows. | **Predicate-input closure — relational & compound** Gate-4 box + strengthened R6 (relational two-operand · compound-AC decomposition · `Producer/Type.field` · generic STOP) + safety-action provenance box — [authoring-standard/50-story-contract.md](authoring-standard/50-story-contract.md); plan-delivery predicate-input preflight — [delivery-pipeline/30-plan-delivery.md](delivery-pipeline/30-plan-delivery.md); principle *Readiness is reconstructed, not asserted* — [authoring-standard/10-principles.md](authoring-standard/10-principles.md) | covered |
...
| LSN-32 | orchestrated-delivery operator UX retro | Prose-only communication guidance drifted under execution pressure: status narration became hard to separate from raw worker/tool events, warning noise, and short-wait filler. Runtime roles need a fixed operator ledger grammar plus an audit over operator-visible updates, not only intent-level prose. | OD-8 fixed `State:` / `Next:` / `Env:` ledger grammar, conservative known-benign warning handling, wait cadence, and post-run communication audit — [delivery-pipeline/40-orchestrated-delivery.md](delivery-pipeline/40-orchestrated-delivery.md) | covered |
| LSN-33 | Epic 4 closure audit / PR #151 | A required produced record/event field or public symbol had no declared source reachable from the story's inputs, owned files, or minting rules. Workers cannot choose the authority for timestamps, refs, ids, or public exports during implementation; missing source authority is a planning/design closure defect. A pure/value classifier story that owns writer/append obligations without explicitly owning the writer seam is the same producer/source-closure defect plus a boundary contradiction. | Producer-closure / produced-obligations rule and pure/value classifier boundary box - [authoring-standard/50-story-contract.md](authoring-standard/50-story-contract.md); story template produced-obligations table - [authoring-standard/_templates/story-contract.md](authoring-standard/_templates/story-contract.md); `plan-epic` producer-closure coverage and characterization review - [../../.agents/skills/plan-epic/SKILL.md](../../.agents/skills/plan-epic/SKILL.md); principle *Readiness is reconstructed, not asserted* - [authoring-standard/10-principles.md](authoring-standard/10-principles.md) | covered |
| LSN-34 | Epic 4 / PR #167 hardening | Repeated orphaned manifest obligations survived because manifest items were mapped to prose responsibilities or one-off evidence instead of the full chain `manifest item -> AC -> standing gate lane`. | Internal coverage both ways + Gate 4 manifest coverage box — [authoring-standard/50-story-contract.md](authoring-standard/50-story-contract.md); manifest/gate-lane preflight — [delivery-pipeline/30-plan-delivery.md](delivery-pipeline/30-plan-delivery.md). Evidence-pack range/artifact-id omissions remain LSN-15 / Gate 5 coverage hardening, not LSN-34. | covered |
