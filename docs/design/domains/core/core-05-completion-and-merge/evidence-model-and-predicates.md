...
| `ForgeActionRefused` is exact-head and retryable, such as transient rate limit, temporary queue unavailable, or update-branch required while policy allows another evidence refresh. | `post-merge-retryable-refused` | From `settling` back to `merge_waiting` through the recovery-classified retry edge. |
