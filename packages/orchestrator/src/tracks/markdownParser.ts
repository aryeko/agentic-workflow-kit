...
import {
  ...
  type ColumnIndexes,
  type ParsedRow,
  type ParseTrackerStoriesContext,
  type TableRow,
  UNOWNED_MARKERS,
...
export function normalizeOwner(value: string): string | null {
  const owner = stripMarkdown(value);
  return UNOWNED_MARKERS.has(owner) ? null : owner;
}

export function readOptionalCell(cells: string[], index: number): string | undefined {
...
function _trackIdFromPath(tracksRoot: string, readmePath: string): string {
  const relativeDir = slash(path.relative(tracksRoot, path.dirname(readmePath)));
  return relativeDir === '' ? 'root' : relativeDir;
}

function _titleFromTrackId(trackId: string): string {
  return trackId
    .split(/[/-]/)
    .filter(Boolean)
...
export function stripMarkdown(value: string | undefined): string {
  return (value ?? '')
    .replace(/\*\*/g, '')
    .replace(/\[([^\]]+)]\([^)]+\)/g, '$1')
    .trim();
...
export function extractTableRows(markdown: string, trackerPath: string): TableRow[] {
  const lines = markdown.split(/\r?\n/);
...
export function hasStoryTableMarkers(cells: string[]): boolean {
  const normalized = cells.map((cell) => normalizeHeader(cell));
  return normalized.includes('id') && (normalized.includes('status') || normalized.includes('name'));
}

function _isPathUnder(candidate: string, parent: string): boolean {
