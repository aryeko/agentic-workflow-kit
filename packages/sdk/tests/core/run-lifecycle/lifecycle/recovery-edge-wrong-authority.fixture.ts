...
export const recoveryEdgeWrongAuthorityFixtures: RunLifecycleTransitionPayload[] = [
  makeTransitionPayload({
    ...
    to: 'running',
    authority: 'system',
    sourceEventIds: ['RecoveryClassified:evt-1'],
  }),
  makeTransitionPayload({
    from: 'forge-waiting',
    to: 'runner-verifying',
    authority: 'system',
    sourceEventIds: ['RecoveryActionPlanned:evt-2'],
  }),
  makeTransitionPayload({
    from: 'merge-waiting',
    to: 'forge-waiting',
    authority: 'system',
    sourceEventIds: ['RecoveryActionApplied:evt-3'],
  }),
  makeTransitionPayload({
    from: 'settling',
    to: 'merge-waiting',
    authority: 'system',
    sourceEventIds: ['ReconciliationBlocked:evt-4'],
  }),
  makeTransitionPayload({
    from: 'runner-verifying',
    to: 'running',
