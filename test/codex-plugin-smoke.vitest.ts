...
describe('codex local plugin smoke', () => {
  it('installs through the local marketplace and exposes implicit-safe skills', () => {
    ...
    execFileSync('codex', ['plugin', 'add', 'agentic-workflow-kit@agentic-workflow-kit-local'], {
      cwd: repoRoot,
      env,
      encoding: 'utf8',
      stdio: 'pipe',
    ...
    const installedRoot = path.join(codexHome, 'plugins/cache/agentic-workflow-kit-local/agentic-workflow-kit/0.1.0');
